...
const MEDIA_CONFIG: Record<MediaType, { icon: typeof Film; label: string; basePath: string }> = {
  movie: { icon: Film, label: 'movie', basePath: '/media/movies' },
  show: { icon: Tv, label: 'show', basePath: '/media/shows' },
};
...
interface FeaturedMediaLoadingProps {
  type: MediaType;
}

function FeaturedMediaLoading({ type }: FeaturedMediaLoadingProps) {
  const { icon: Icon } = MEDIA_CONFIG[type];
    ...
    <section className="relative overflow-hidden rounded-xl">
      <div className="relative aspect-[4/3] w-full sm:aspect-[16/9] md:aspect-[2/1] xl:aspect-[2.4/1]">
        ...
        <div className="absolute right-6 top-6 hidden animate-pulse text-muted-foreground/10 md:block lg:right-12 lg:top-12">
          <Icon className="size-24 lg:size-32" strokeWidth={1} />
        </div>
      </div>
    </section>
  );
...
function FeaturedMediaError({ type, error, refetch }: FeaturedMediaErrorProps) {
  const { label } = MEDIA_CONFIG[type];
...
interface FeaturedMediaSlideProps {
  item: FeaturedMediaItem;
  basePath: string;
  isActive: boolean;
}

function FeaturedMediaSlide({ item, basePath, isActive }: FeaturedMediaSlideProps) {
  return (
    <div
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground sm:gap-3 sm:text-sm">
            {item.rating > 0 && (
              <span className="flex items-center gap-1.5 text-yellow-400">
                ...
                <span className="font-medium tabular-nums">{item.rating.toFixed(1)}</span>
              </span>
            )}
...
function FeaturedMediaCarousel({ type, items }: FeaturedMediaCarouselProps) {
  const { basePath } = MEDIA_CONFIG[type];
...
export type { FeaturedMediaCarouselProps, FeaturedMediaErrorProps, FeaturedMediaItem, FeaturedMediaLoadingProps };
export { FeaturedMediaCarousel, FeaturedMediaError, FeaturedMediaLoading };
