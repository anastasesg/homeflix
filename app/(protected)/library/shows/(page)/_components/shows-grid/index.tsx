...
import { AlertCircle, CheckCircle2, Clock, Download, Tv2 } from 'lucide-react';

import { ShowItemsResponse } from '@/api/functions';
import { DisplayStatus } from '@/api/entities';
import { useShowFilters, type ShowTabValueType } from '@/hooks/filters';
...
function ShowsGridSuccess({ shows, stats, isRefetching }: ShowsGridSuccessProps) {
  ...
  const showStatusConfig: Record<DisplayStatus, StatusConfig> = {
    complete: {
      icon: CheckCircle2,
      label: 'Complete',
      ...
      <TabsContent value={filters.tab} className="mt-6">
        <div className="relative">
          ...
          <MediaGrid
            ...
            renderCard={(show, index) => (
              <ShowCard key={show.id} show={show} status={showStatusConfig[show.status]} index={index} />
            )}
            renderListItem={(show) => <ShowItem key={show.id} show={show} status={showStatusConfig[show.status]} />}
          />
        </div>
      </TabsContent>
    </Tabs>
