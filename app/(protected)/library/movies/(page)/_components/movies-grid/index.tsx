...
import { AlertCircle, CheckCircle2, Clock, Download, Film } from 'lucide-react';
...
function MoviesGridSuccess({ movies, stats, isRefetching }: MoviesGridSuccessProps) {
  ...
  const movieStatusConfig: Record<MediaStatus, StatusConfig> = {
    downloaded: {
      icon: CheckCircle2,
      label: 'Downloaded',
      ...
      <TabsContent value={filters.tab} className="mt-6">
        <div className="relative">
          ...
          <MediaGrid
            ...
            renderCard={(movie) => <MovieCard key={movie.id} movie={movie} status={movieStatusConfig[movie.status]} />}
            renderListItem={(movie) => (
              <MovieItem key={movie.id} movie={movie} status={movieStatusConfig[movie.status]} />
            )}
          />
        </div>
      </TabsContent>
