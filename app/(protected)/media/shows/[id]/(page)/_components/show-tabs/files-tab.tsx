...
function getQualityColor(quality: string): string {
  ...
  if (q.includes('2160') || q.includes('4k') || q.includes('uhd')) return 'from-amber-500 to-orange-600';
  if (q.includes('1080')) return 'from-blue-500 to-indigo-600';
  if (q.includes('720')) return 'from-emerald-500 to-teal-600';
...
function getCodecIcon(codec: string): { icon: string; color: string } {
  ...
  if (c.includes('x265') || c.includes('hevc')) return { icon: 'H.265', color: 'text-emerald-400' };
  if (c.includes('x264') || c.includes('avc')) return { icon: 'H.264', color: 'text-blue-400' };
  if (c.includes('av1')) return { icon: 'AV1', color: 'text-purple-400' };
...
function formatEpisodeBadge(seasonNumber: number, episodeNumber: number): string {
  return `S${String(seasonNumber).padStart(2, '0')}E${String(episodeNumber).padStart(2, '0')}`;
}

function groupFilesBySeason(files: EpisodeFile[]): Map<number, EpisodeFile[]> {
  const groups = new Map<number, EpisodeFile[]>();
  for (const file of files) {
    const existing = groups.get(file.seasonNumber);
    if (existing) {
      existing.push(file);
    } else {
      groups.set(file.seasonNumber, [file]);
    }
  }
...
function FileCard({ file, showSeasonBadge }: FileCardProps) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="shrink-0 border-border/60 font-mono text-xs">
                {showSeasonBadge
                  ? formatEpisodeBadge(file.seasonNumber, file.episodeNumber)
                  : `E${String(file.episodeNumber).padStart(2, '0')}`}
              </Badge>
...
function SeasonGroup({ seasonNumber, files }: SeasonGroupProps) {
  const [open, setOpen] = useState(true);
  const sortedFiles = [...files].sort((a, b) => a.episodeNumber - b.episodeNumber);

  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center gap-2 rounded-lg border border-border/40 bg-muted/20 px-4 py-2.5 text-left transition-colors hover:border-border hover:bg-muted/40">
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/50 bg-muted/20 py-16 text-center">
      ...
      <p className="mt-2 max-w-xs text-sm text-muted-foreground">
        No episode files have been downloaded for this show yet.
      </p>
    </div>
  );
}
...
function FilesTabContent({ files }: FilesTabContentProps) {
  const seasonGroups = useMemo(() => groupFilesBySeason(files), [files]);

  if (files.length === 0) return <EmptyFiles />;

  return (
    <div className="space-y-6">
      {[...seasonGroups.entries()].map(([seasonNumber, seasonFiles]) => (
        <SeasonGroup key={seasonNumber} seasonNumber={seasonNumber} files={seasonFiles} />
      ))}
    </div>
  );
}
...
function FilesTab({ tmdbId }: FilesTabProps) {
  ...
  return (
    <Queries
      results={[libraryQuery, filesQuery] as const}
      callbacks={{
        loading: FilesTabLoading,
        error: () => null,
        success: ([_library, files]) => <FilesTabContent files={files} />,
      }}
    />
  );
}
