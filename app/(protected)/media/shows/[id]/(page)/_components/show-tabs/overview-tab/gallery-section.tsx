...
function Lightbox({ images, initialIndex, showTitle, open, onOpenChange, aspectRatio }: LightboxProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogTitle className="sr-only">
          {showTitle} — Image {currentIndex + 1} of {images.length}
        </DialogTitle>
...
function GallerySectionContent({ images, showTitle }: GallerySectionContentProps) {
        ...
        <CarouselContent className="-ml-2 py-1">
          {currentImages.map((url, index) => (
            <GalleryItem
              key={url}
              url={url}
              alt={`${showTitle} ${activeTab === 'backdrops' ? 'backdrop' : 'poster'} ${index + 1}`}
              index={index}
