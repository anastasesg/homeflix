...
const mockSearchResults: SearchResult[] = [
  {
    id: 1,
    title: 'The.Bear.S03E10.1080p.WEB.x264-GROUP',
    indexer: 'NZBgeek',
    size: '1.4 GB',
    seeders: 156,
    quality: 'WEBDL-1080p',
  ...
  {
    id: 2,
    title: 'The.Bear.S03E10.720p.WEB.x264-GROUP',
    indexer: 'Drunkenslug',
    size: '850 MB',
    seeders: 89,
    quality: 'WEBDL-720p',
  ...
  {
    id: 3,
    title: 'The.Bear.S03.COMPLETE.1080p.WEB.x265-GROUP',
    indexer: 'NZBFinder',
    size: '12.4 GB',
    seeders: 234,
    quality: 'WEBDL-1080p',
  ...
  {
    id: 4,
    title: 'The.Bear.S03E10.480p.WEB-DL',
    indexer: 'NZBFinder',
    size: '320 MB',
    seeders: 45,
    quality: 'WEBDL-480p',
...
function SearchResultCard({ result, onDownload }: { result: SearchResult; onDownload: () => void }) {
  ...
  const hasEpisode = result.seasonNumber !== undefined && result.episodeNumber !== undefined;
  ...
  return (
    ...
    >
      ...
      <div className="p-4 pl-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {hasEpisode && (
                <Badge variant="outline" className="border-white/20 font-mono text-[10px]">
                  S{result.seasonNumber!.toString().padStart(2, '0')}E
                  {result.episodeNumber!.toString().padStart(2, '0')}
                </Badge>
              )}
              {!hasEpisode && (
        ...
        {/* Size bar */}
        <div className="mt-3">
          <Progress value={Math.min((sizeGB / 15) * 100, 100)} className="h-1 bg-white/[0.04]" />
        </div>
      </div>
    </div>
  );
...
interface SeasonMonitoringCardProps {
  season: {
    id: number;
    seasonNumber: number;
...
function SeasonMonitoringCard({ season }: SeasonMonitoringCardProps) {
  const [monitored, setMonitored] = useState(true);
  const progress = season.episodeCount > 0 ? Math.round((season.downloadedCount / season.episodeCount) * 100) : 0;
  const isComplete = season.downloadedCount === season.episodeCount;

  return (
    <div className="group flex items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 transition-colors hover:bg-white/[0.03]">
      <div className="flex size-9 items-center justify-center rounded-lg bg-amber-500/10">
        <span className="text-sm font-semibold text-amber-500">{season.seasonNumber}</span>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-1">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isComplete && <Badge className="bg-emerald-500/20 text-[10px] text-emerald-400">Complete</Badge>}
            <span className="text-xs tabular-nums text-muted-foreground">
              {season.downloadedCount}/{season.episodeCount}
            </span>
          </div>
        </div>
...
export function ManageTab({ show }: ManageTabProps) {
  const [monitored, setMonitored] = useState(show.monitored);
  const [qualityProfileId, setQualityProfileId] = useState(String(show.qualityProfileId));
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <SettingCard
          ...
          title="Monitoring"
          description="Automatically search for new episodes and upgrades"
        >
      ...
      <div>
        <div className="mb-3 flex items-center gap-2">
          <Tv2 className="size-4 text-amber-500" />
          <h3 className="text-sm font-semibold uppercase tracking-wider">Season Monitoring</h3>
        </div>
        <div className="space-y-2">
          {show.seasons
            .sort((a, b) => b.seasonNumber - a.seasonNumber)
            .map((season) => (
              <SeasonMonitoringCard key={season.id} season={season} />
            ))}
        </div>
      </div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {searchResults.length > 0
                    ...
                    : 'Click Search All to find releases'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
        ...
        <CollapsibleContent className="pt-4">
          {isSearching ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/[0.08] py-12 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">Click Search All above to find available releases</p>
            </div>
          )}
        </CollapsibleContent>
      </Collapsible>
