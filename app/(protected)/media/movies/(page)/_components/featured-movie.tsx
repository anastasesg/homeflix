...
import { AlertCircle, Film, Play, RefreshCw, Star } from 'lucide-react';
...
interface FeaturedMovieErrorProps {
  error: Error;
  refetch: () => void;
}

function FeaturedMovieError({ error, refetch }: FeaturedMovieErrorProps) {
  return (
    <section className="relative overflow-hidden rounded-xl">
      <div className="relative aspect-[4/3] w-full sm:aspect-[16/9] md:aspect-[2/1] xl:aspect-[2.4/1]">
...
function FeaturedMovieSlide({ movie, isActive }: FeaturedMovieSlideProps) {
  return (
    <div
...
function FeaturedMovieCarousel({ movies }: FeaturedMovieCarouselProps) {
  const slides = movies.slice(0, HERO_COUNT);
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] w-full sm:aspect-[16/9] md:aspect-[2/1] xl:aspect-[2.4/1]">
        {slides.map((movie, i) => (
          <FeaturedMovieSlide key={movie.id} movie={movie} isActive={i === activeIndex} />
        ))}
  ...
  return (
    <Query
      ...
      callbacks={{
        ...
        error: (error) => <FeaturedMovieError error={error} refetch={trendingQuery.refetch} />,
        success: (movies) => <FeaturedMovieCarousel movies={movies} />,
      }}
    />
  );
}
