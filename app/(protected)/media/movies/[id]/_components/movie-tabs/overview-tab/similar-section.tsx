...
import { similarMoviesQueryOptions } from '@/options/queries/movies/detail';
...
function SimilarSectionContent({ movies }: { movies: MovieRecommendation[] }) {
  if (movies.length === 0) return null;

  return (
    <section>
      <SectionHeader icon={Shuffle} title="Similar Movies" count={movies.length} />
  ...
  return (
    <Query
      ...
      callbacks={{
        ...
        error: () => null,
        success: (movies) => <SimilarSectionContent movies={movies} />,
      }}
    />
  );
}
